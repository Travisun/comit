  ...
  return (
    <div className="min-h-dvh w-full py-[10px]">
      ...
      <section className="border-b border-border">
        <h2 className="px-5 pb-1 pt-3 text-[15px] font-bold">{t("user.topicCloud")}</h2>
        {topics.length === 0 ? (
      ...
      {/* hot posts */}
      <section className="border-b border-border">
        <h2 className="px-5 pb-1 pt-3 text-[15px] font-bold">{t("home.trending")}</h2>
        {hot.length === 0 ? (
          <Empty text="暂无热门内容。" />
        ) : (
          <ol className="px-5">
      ...
      {/* active authors */}
      <section>
        <h2 className="px-5 pb-1 pt-3 text-[15px] font-bold">{t("home.featuredAuthors")}</h2>
        {authors.length === 0 ? (
          <Empty text="还没有活跃作者。" />
        ) : (
          <ul className="px-5">
            {authors.map((a) => (
              <li key={a.username}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[15px] font-bold">{a.displayName}</span>

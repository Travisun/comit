 ...
 *
 * ============================ 设置项权限契约 ============================
 * 扩展配置分两级，存储、端点与界面一一对应，互不可越界写入：
 *
 *  | 级别   | 存储                          | 读写端点                     | 界面                          |
 *  |--------|-------------------------------|------------------------------|-------------------------------|
 *  | 用户级 | users.extSettings[<extId>]（按用户行隔离） | /api/me/ext/<id>/settings（withUser，仅本人行） | 设置 → 扩展 → [扩展] 子页（三级） |
 *  | 管理员级 | settings 表 ext.* / 全局键     | /api/admin/settings（withAdmin） | 管理后台各设置面板              |
 *
 *  - settingsFields 声明的就是**用户级**字段；未声明 settingsFields 的扩展
 *    不出现在前台 设置 → 扩展（纯后台能力扩展只走管理员级配置）。
 *  - 用户级写入一律经 coerceExtSettings 白名单收敛（未声明键丢弃 / 类型
 *    回落默认 / 选项白名单 / maxLength 截断），篡改面在服务端收口。
 *  - 扩展启停（ext.enabled）与全局参数属管理员级，用户端点不可达。
 * =======================================================================
 */
